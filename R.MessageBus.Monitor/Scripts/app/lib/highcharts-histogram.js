...
(function(H) {
    H.wrap(H.Chart.prototype, 'init', function(proceed, userOptions, callback) {
        if (userOptions.chart.type === undefined || userOptions.chart.type === null || userOptions.chart.type.toLowerCase() !== 'histogram')
        ...
        this.series[0].data.forEach(function(d, i) {
            d.histBin = i;
            d.histFrom = moment.utc(Number((d.x - binWidth / 2).toFixed(12))).format("DD/MM/YYYY HH:mm:ss");
            d.histTo = moment.utc(Number((d.x + binWidth / 2).toFixed(12))).format("DD/MM/YYYY HH:mm:ss");
        });
    });
